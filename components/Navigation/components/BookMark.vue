    ...
    <div>
        <v-tooltip bottom>
            <template #activator="{ on, attrs }">
                <v-btn icon v-bind="attrs" @click="showBookmark" v-on="on">
                    <v-icon>far fa-bookmark</v-icon>
                    <v-icon x-small class="bookmark-config-icon"
                        >fas fa-cog</v-icon
                    >
                </v-btn>
            </template>
        ...
        <CommonModal v-model="bookmarkShow" name="bookmark" class="transparent">
            <div class="py-2">
                ...
                <div class="modal-footer">
                    <v-btn @click="hideBookmark"> 返回 </v-btn>
                    <v-spacer></v-spacer>
                    <v-btn
                        v-if="!pageHasMarked"
                        color="success"
...
export default {
    ...
    methods: {
        ...
        updateHasMarked() {
            this.pageHasMarked = this.bmList.find(
                (mark) => mark.path === location.pathname + location.hash
            )
        },
        showBookmark() {
