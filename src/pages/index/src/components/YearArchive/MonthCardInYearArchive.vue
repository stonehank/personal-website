        ...
        <v-row class="h-100">
            <v-col cols="4" md="3" v-for="(percent,index) in percentH" :key="index">
                <div ref="month-card"
                    ...
                    >
                        <h4 class="month-count-text text-lg">{{index+1}} 月</h4>
                        <AutoGrowLine
                                :finalHeight="percent"
                                :start="start"
...
<script>
    import AutoGrowLine from "pagesDir/index/src/components/YearArchive/AutoGrowLine";
    ...
    export default {
        ...
        computed:{
            percentH:function(){
                let max=Math.max.apply(Math,this.counts)
                return this.counts.map(count=>count / max * 100)
            },
            bgColors:function(){
                let colors=['#caffb1','#9fff90','#6ffc6f','#48db5c','#24d648','#02c215']
                let max=Math.max.apply(Math,this.counts)
